...
_setupVars =
{
	_missionType = "Annexation of Industry";
...
_setupObjects =
{
	...
	_camonet = createVehicle ["Flag_POWMIA_F", [_missionPos select 0, _missionPos select 1], [], 0, "CAN COLLIDE"];
	...
	_obj1 = createVehicle ["Land_Device_assembled_F", _missionPos,[], 10,"None"]; 
	_obj1 setPosATL [(_missionPos select 0) - 2, (_missionPos select 1) + 2, _missionPos select 2];
	
	_obj2 = createVehicle ["I_HMG_01_A_F", _missionPos,[], 10,"None"]; 
	...
	_missionHintText = format ["<br/>Paramilitary units want to <t color='%1'>annex local industry areas</t>.<br/>Intercept their plans, kill these vultures and keep the area clean!<br/>For fuck's sake ..!", occupationMissionColor];
};
...
_successExec =
{
	...
	_successHintMessage = format ["<br/>Hell yeah! You kicked these asses out of the industrial area! Well done!"];
};
